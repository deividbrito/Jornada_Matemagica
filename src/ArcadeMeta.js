...
class ArcadeMeta {
  constructor() {
    // Mantido por compatibilidade — todas as chamadas agora passam pelo apiClient.
    this.apiBase = "/api/arcade";
  ...
  _idJogador() {
    // Sem token válido, qualquer ID em sessaoData é tratado como guest —
    // evita chamar endpoints autenticados sem credencial.
    if (!window.api || !window.api.isLogged()) return null;
  ...
  async getMeta() {
    ...
    try {
      const data = await window.api.fetch(`${this.apiBase}/meta/${id}`);
  ...
  async getHistory(limit = 10, sort = "recent") {
    ...
    try {
      return await window.api.fetch(`${this.apiBase}/historico/${id}?limit=${limit}&sort=${sort}`);
    } catch (err) {
  ...
  // Requer autenticação — se não logado, devolve estrutura vazia.
  async getGlobalRanking(limit = 10) {
    ...
    try {
      return await window.api.fetch(`${this.apiBase}/ranking?limit=${limit}&id_jogador=${id}`);
    } catch (err) {
  ...
  async recordRun(runResult) {
    ...
    try {
      // id_jogador/id_sessao são deduzidos do JWT no backend — não enviamos.
      const body = {
      ...
      const data = await window.api.fetch(`${this.apiBase}/runs`, {
        method: "POST",
